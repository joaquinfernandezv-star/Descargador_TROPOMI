...
# tkcalendar is needed for date pickers
try:
    from tkcalendar import DateEntry
except ImportError:
    messagebox.showerror("Missing Library", "The 'tkcalendar' library is not installed.\n\nPlease install it by running in your terminal:\npip install tkcalendar")
    sys.exit(1)

# --- Geospatial and Scientific Imports ---
try:
    import matplotlib
    ...
    messagebox.showerror("Missing Library", f"A required library is missing: {e}.\n\nPlease install it using 'pip install <library>'.")
    sys.exit(1)

# ==============================================================================
# SECTION 1: PROCESS LOGIC AND CONFIGURATION
# ==============================================================================
...
# Keeping original folder names to prevent data loss or disconnections
...
# --- GAS CONFIGURATION ---
GAS_CONFIG = {
    "NO2": { "nombre_corto": "NO2", "nombre_largo": "Nitrogen Dioxide", "band_name": "NO2", "peso_molecular": 46.01, "factor_ppb": 24.45, "comportamiento_vertical": "PBL_confined" },
    "O3": { "nombre_corto": "O3", "nombre_largo": "Ozone", "band_name": "O3", "peso_molecular": 48.00, "factor_ppb": 24.45, "comportamiento_vertical": "Thick_troposphere" },
    "SO2": { "nombre_corto": "SO2", "nombre_largo": "Sulfur Dioxide", "band_name": "SO2", "peso_molecular": 64.07, "factor_ppb": 24.45, "comportamiento_vertical": "PBL_confined" },
    "CO": { "nombre_corto": "CO", "nombre_largo": "Carbon Monoxide", "band_name": "CO", "peso_molecular": 28.01, "factor_ppb": 24.45, "comportamiento_vertical": "Thick_troposphere" },
    "CH4": { "nombre_corto": "CH4", "nombre_largo": "Methane", "band_name": "CH4", "peso_molecular": 16.04, "factor_ppb": 24.45, "comportamiento_vertical": "Thick_troposphere" },
    "HCHO": { "nombre_corto": "HCHO", "nombre_largo": "Formaldehyde", "band_name": "HCHO", "peso_molecular": 30.03, "factor_ppb": 24.45, "comportamiento_vertical": "PBL_confined" }
}
...
try:
    ...
    if not config.sh_client_id or not config.sh_client_secret:
        print("Configuring Copernicus Data Space credentials...")
    ...
    print(f"Error configuring SentinelHub: {e}")
...
meses_en_lower = {k: v.lower() for k, v in meses_dict.items()}
...
paletas_colores = {
    'viridis': 'Purple to yellow (Standard)', 'plasma': 'Purple to warm yellow', 
    'inferno': 'Black, red, yellow', 'magma': 'Black, purple, yellow',
    'cividis': 'Blue to yellow (Colorblind)', 'turbo': 'Enhanced rainbow',
    'Spectral': 'Red to Blue (Diverging)', 'RdYlGn': 'Red, Yellow, Green'
}
if tiene_cmcrameri: paletas_colores['batlow'] = 'Perceptually uniform (Batlow)'
...
def calcular_estadisticas_nubosidad(time_start, time_end, route):
    print(f"☁️  Analyzing cloud cover for {time_start} - {time_end}...")
    ...
    try:
        ...
        print(f"❌ Error calculating cloud cover: {e}")
        return None
...
def descargar_y_promediar_gas(time_start, time_end, route, output_name, gas_info, cancel_event, generar_serie=False):
    ...
    final_folder = BASE_OUTPUT_PATH / f"Model_{gas_name}/{year_folder}/{area}/{output_name}"
                    ...
                    try:
                        with rasterio.open(tiff_p) as src:
                            ...
                            if val.size > 0:
                                ...
                                csv_path = final_folder / f"Time_Series_Range_{gas_name}_{time_start}.csv"
                                pd.DataFrame([{'Date': time_start, 'P5': p5, 'P50': p50, 'P95': p95}]).to_csv(csv_path, index=False)
    ...
    daily_tiffs = []
    daily_dates = []
    print(f"🔄 Starting daily progressive download for {gas_name} ({delta.days + 1} days)...")
    ...
    for i in range(delta.days + 1):
        ...
        sys.stdout.write(f"\r      Downloading {gas_name} for date: {current_date}... ")
        sys.stdout.flush()
    ...
    print(f"\n✅ {gas_name} downloads completed. Averaging data...")

    # Extract Median and Daily Range Time Series if requested
        ...
        for tiff, d_str in zip(daily_tiffs, daily_dates):
            try:
                with rasterio.open(tiff) as src:
                    ...
                    if val.size > 0:
                        p5, p50, p95 = np.percentile(val, [5, 50, 95])
                        stats_records.append({'Date': d_str, 'P5': p5, 'P50': p50, 'P95': p95})
            except Exception: pass
        
        if stats_records:
            csv_path = final_folder / f"Time_Series_Range_{gas_name}.csv"
            pd.DataFrame(stats_records).to_csv(csv_path, index=False)

    if not daily_tiffs:
        print(f"❌ No valid {gas_name} data found in the selected period.")
        shutil.rmtree(temp_dir, ignore_errors=True)
        return None, None, None, [], []
    ...
    try:
        ...
        print(f"✅ Local average for {gas_name} saved successfully.")
    except Exception as e:
        print(f"❌ Error processing the average: {e}")
        shutil.rmtree(temp_dir, ignore_errors=True)
        return None, None, None, [], []
...
def extraer_serie_ppb(daily_tiffs, daily_dates, ruta_blh_crudo, metodo_transform, valor_custom, gas_info, ruta_geojson, output_csv):
    """Calculates daily PPB iterating over each downloaded image and exports its percentiles."""
    ...
    try:
        ...
        if metodo_transform == "petetin" and ruta_blh_crudo:
            ...
            # Rigorous geometric mask aligned to gas TIFF using in-memory file
        ...
        # Multiplier factor combines conversion (24.45 / MolecularWeight) * (1e6 * MolecularWeight) = 24.45 * 1e6
        ...
        if stats_records:
            ...
            return output_csv
    except Exception as e: print(f"❌ Error extracting PPB time series: {e}")
    return None

def descargar_blh_era5(year, month, area_coords, output_path):
    print("🛰️  Connecting to Copernicus CDS to download BLH (Monthly Average)...")
    c = cdsapi.Client()
    request_payload = {
    ...
    try:
        ...
        print(f"✅ Monthly BLH download completed: {output_path}")
        return True
    except Exception as e:
        print(f"❌ Error during monthly BLH download from CDS: {e}")
        return False

def descargar_blh_era5_diario(start_date, end_date, area_coords, output_path):
    print(f"📡 Requesting Hourly ERA5 (12:00) for {start_date}...")
    c = cdsapi.Client()
    request_payload = {
    ...
    try:
        ...
        print("✅ Daily BLH download completed.")
        return True
    except Exception as e:
        print(f"❌ Error with Daily ERA5: {e}")
        return False
...
def procesar_concentracion_gas(ruta_crudo, ruta_blh_crudo, output_dir, region_nombre, año, mes_nombre_en, gas_info, formato_salida="GeoTIFF", metodo_transform="petetin", valor_custom=None, suffix=""):
    gas_name = gas_info["nombre_corto"]
    print(f"\n---\n🔬 Calculating {gas_name} concentration for: {mes_nombre_en} {año} (Mode: {metodo_transform})")
    ...
    try:
        with rasterio.open(ruta_crudo) as src_gas:
            ...
            nombre_archivo_base = f"Concentration_ppb_{gas_name}_{region_nombre}_{año}_{mes_nombre_en}{suffix}"
            ...
            if formato_salida == "NetCDF4":
                ...
                data_array = template_raster.copy(data=data_con_banda).rename(f'concentration_{gas_name.lower()}_ppb')
            ...
            print(f"✅ Concentration calculation completed: {output_path}")
            return output_path
    except Exception as e:
        print(f"❌ Error processing {gas_name} concentration: {e}")
        return None

# --- RATIO AND MATRIX CORRELATION CALCULATION ---
def calcular_y_guardar_ratio(ruta_num, ruta_den, output_dir, gas_num, gas_den, region, año, mes_str):
    try:
        with rasterio.open(ruta_num) as src_num, rasterio.open(ruta_den) as src_den:
            ...
            # Avoid division by zero and anomalous values (negatives in VCD)
            ...
            output_path = output_dir / f"Ratio_{gas_num}_over_{gas_den}_{region}_{año}_{mes_str}.tiff"
            profile.update(dtype=rasterio.float32, nodata=np.nan)
            
            with rasterio.open(output_path, 'w', **profile) as dst:
                dst.write(ratio_data, 1)

            print(f"✅ Ratio map {gas_num}/{gas_den} generated successfully.")
            return output_path
    except Exception as e:
...
def calcular_y_graficar_correlacion(ruta_num, ruta_den, ruta_geojson, gas_num, gas_den, region_nombre, año, mes_str, output_dir, return_fig=False):
    """Calculates Pearson and Spearman and generates a density plot between two gases"""
    try:
        ...
        with rasterio.open(ruta_num) as src_num, rasterio.open(ruta_den) as src_den:
            ...
            # Extract only valid pixels for both components concurrently
            valid_mask = ~np.isnan(val_n) & ~np.isnan(val_d) & (val_n > 0) & (val_d > 0)
            val_n = val_n[valid_mask]
            val_d = val_d[valid_mask]
            
            if val_n.size < 2:
                print("❌ Not enough concurrent data for correlation.")
                return None
            ...
            # Hexbin plot to handle large data volume (spatial density)
            hb = ax.hexbin(val_d, val_n, gridsize=50, cmap='plasma', mincnt=1, bins='log')
            cb = fig.colorbar(hb, ax=ax)
            cb.set_label('log$_{10}$(Pixel Density)', fontsize=12)
            
            # Crop extreme outliers dynamically adjusting the range on both axes
            p01_d, p99_d = np.percentile(val_d, [1, 99])
            p01_n, p99_n = np.percentile(val_n, [1, 99])
            
            # Add a visual margin of 5%
            margen_d = (p99_d - p01_d) * 0.05
            margen_n = (p99_n - p01_n) * 0.05
            
            xlim_min = max(0, p01_d - margen_d) # In concentrations the logical min is ~0
            ...
            ax.plot(x_trend, m*x_trend + b, color='lime', linestyle='--', linewidth=2, label=f'Linear Trend')
            
            ax.set_title(f"Spatial Correlation: {gas_num} vs {gas_den}\n{region_nombre.replace('_', ' ').title()} • {mes_str.capitalize()} {año}", fontsize=15, pad=15)
            ax.set_xlabel(f"X Axis: [{gas_den}] {gas_den_largo} (mol/m²)", fontsize=13)
            ax.set_ylabel(f"Y Axis: [{gas_num}] {gas_num_largo} (mol/m²)", fontsize=13)
            
            # Metrics box with clear identification
            textstr = (
                f"Gas Identification:\n"
                f"$\\bullet$ Y Axis: {gas_num} ({gas_num_largo})\n"
                f"$\\bullet$ X Axis: {gas_den} ({gas_den_largo})\n"
                f"--------------------------------------\n"
                f"Pearson ($r$): {corr_p:.3f}\n"
                f"Spearman ($\\rho$): {corr_s:.3f}\n"
                f"Valid Pixels: {val_n.size}"
            )
            ...
            # Save to specified period folder
            output_dir = Path(output_dir)
            output_dir.mkdir(parents=True, exist_ok=True)
            output_path = output_dir / f"Correlation_{gas_num}_vs_{gas_den}_{region_nombre}_{año}_{mes_str}.png"
...
def graficar_serie_temporal(csv_paths, region_nombre, gas_nombre, title_suffix, output_dir, is_ppb=False, return_fig=False):
    """Plots evolution with a shaded area for the dispersion range (P5 to P95) and median as a solid line"""
    ...
    # Shading area between P5 and P95
    ax.fill_between(df_all['Date'], df_all['P5'], df_all['P95'], color='dodgerblue', alpha=0.3, label='Dispersion Range (P5 - P95)')
    
    # Solid line for Median (P50)
    ax.plot(df_all['Date'], df_all['P50'], marker='o', linestyle='-', color='dodgerblue', markersize=5, linewidth=2, label='Spatial Median (P50)')
    
    if is_ppb:
        ax.set_title(f"Temporal Evolution (Surface PPB) - {gas_nombre}\n{region_nombre.replace('_', ' ').title()} • {title_suffix}", fontsize=14)
        ax.set_ylabel(f"Surface Concentration {gas_nombre} (ppb)", fontsize=12)
        out_path = output_dir / f"Time_Series_Range_PPB_{gas_nombre}_{title_suffix.replace(' ', '_')}.png"
    else:
        ax.set_title(f"Temporal Evolution (Total Column) - {gas_nombre}\n{region_nombre.replace('_', ' ').title()} • {title_suffix}", fontsize=14)
        ax.set_ylabel(f"Column Concentration {gas_nombre} (mol/m²)", fontsize=12)
        out_path = output_dir / f"Time_Series_Range_Column_{gas_nombre}_{title_suffix.replace(' ', '_')}.png"
...
def graficar_serie_temporal_dual(csv_paths_num, csv_paths_den, region_nombre, gas_num, gas_den, title_suffix, output_dir, return_fig=False):
    """Plots a dual-axis evolution of two gases during Ratio Map analysis using Median and Shading"""
    ...
    df_den = df_den.sort_values('Date')
    
    fig, ax1 = plt.subplots(figsize=(12, 6))
    
    # Primary Y axis (Numerator Gas)
    color1 = 'tab:blue'
    ax1.set_xlabel('Date', fontsize=12)
    ax1.set_ylabel(f'{gas_num} (mol/m²)', color=color1, fontsize=12)
    ax1.fill_between(df_num['Date'], df_num['P5'], df_num['P95'], color=color1, alpha=0.2)
    line1 = ax1.plot(df_num['Date'], df_num['P50'], color=color1, marker='o', linestyle='-', markersize=4, linewidth=2, label=f'{gas_num} (Median & P5-P95)')
    ax1.tick_params(axis='y', labelcolor=color1)
    plt.xticks(rotation=45)
    
    # Secondary Y axis (Denominator Gas)
    ax2 = ax1.twinx()
    color2 = 'tab:red'
    ax2.set_ylabel(f'{gas_den} (mol/m²)', color=color2, fontsize=12)
    ax2.fill_between(df_den['Date'], df_den['P5'], df_den['P95'], color=color2, alpha=0.2)
    line2 = ax2.plot(df_den['Date'], df_den['P50'], color=color2, marker='s', linestyle='-', markersize=4, linewidth=2, label=f'{gas_den} (Median & P5-P95)')
    ...
    fig.suptitle(f"Comparative Temporal Evolution (Spatial Median & Dispersion Range)\n{region_nombre.replace('_', ' ').title()} • {title_suffix}", fontsize=14)
    fig.tight_layout()
    fig.subplots_adjust(bottom=0.2) # Extra space for bottom legend
    
    out_path = output_dir / f"Time_Series_Range_Dual_{gas_num}_vs_{gas_den}_{title_suffix.replace(' ', '_')}.png"
    plt.savefig(out_path, dpi=300)
    print(f"📈 Comparative time series saved to: {out_path}")
...
def generar_mapa_con_leyenda(ruta_tiff, ruta_geojson, title_date, year, cmap='viridis', alpha=0.75, producto="Data", unidad="Unit", return_fig=False):
    try:
        ...
        procesamiento = "Rescaled (Kriging)" if 'regrid' in Path(ruta_tiff).stem.lower() else ("Calculated" if 'concentracion' in Path(ruta_tiff).stem.lower() or 'concentration' in Path(ruta_tiff).stem.lower() else "Raw")

        fig, ax = plt.subplots(figsize=(12, 10))
        cmap_obj = cmc.batlow if tiene_cmcrameri and cmap == 'batlow' else plt.get_cmap(cmap)
        
        # --- NEW MAP LOGIC FOR DYNAMIC SCALING ---
        ...
        if "Relación" in producto or "Ratio" in producto:
            vmin = max(0, np.nanmin(data)) # Ratios usually start from 0
            if "HCHO" in producto and "NO2" in producto:
                # Cap hard at 5 or less so we don't wash out sensitivity (0 to ~2 is the threshold of interest)
                vmax_real = min(5.0, max(2.0, p98))
            else:
                # For generic ratios, P95 is safer to avoid infinity-bound values
                vmax_real = p95
        else:
            vmin = p02
            vmax_real = p98
        ...
        # Add explanatory legend for regimes if it's HCHO/NO2
        if "HCHO" in producto and "NO2" in producto:
            props = dict(boxstyle='round', facecolor='white', alpha=0.85, edgecolor='gray')
            regimes_text = (
                "Photochemical Sensitivity Index\n"
                "HCHO (Formaldehyde) / NO2 (Nitrogen Dioxide)\n"
                "--------------------------------------------------\n"
                "$\\bullet$ Ratio < 1 : VOC-limited regime\n"
                "$\\bullet$ Ratio 1 - 2 : Transition zone\n"
                "$\\bullet$ Ratio > 2 : NOx-limited regime"
            )
        ...
        ax.set_title(f"{producto} in {region_nombre} • {title_date.capitalize()} {year}\n{procesamiento} Data", fontsize=16, pad=15)
        ax.set_xlabel("Longitude", fontsize=14); ax.set_ylabel("Latitude", fontsize=14); plt.setp(ax.get_xticklabels(), rotation=45, ha="right")
    ...
    except Exception as e: print(f"❌ Error generating map: {e}"); return None
...
def generar_mapa_comparativo(file_paths, aoi_path, producto, unidad, cmap, title_suffix, output_dir=None, return_fig=False):
    ...
    try:
        ...
        # --- ROBUST GLOBAL LIMITS CALCULATION ---
        ...
        for i, file_path in enumerate(file_paths):
            ...
            else: ax.set_title(f"{title}\n(No data)", fontsize=12); ax.set_facecolor('0.95') 
        ...
        fig.suptitle(f"{producto} in {region_nombre} • {title_suffix}", fontsize=18) 
        ...
        if img: 
            ...
            fig.supxlabel('Longitude', fontsize=16, y=0.02); fig.supylabel('Latitude', fontsize=16, x=0.01)
        ...
        if output_dir is None:
            ...
            output_dir = Path(output_dir)
        output_dir.mkdir(parents=True, exist_ok=True)
        output_path = output_dir / f"Comparative_Map_{producto.split()[0]}_{title_suffix.replace(' ', '_')}.png"
        
        plt.savefig(output_path, bbox_inches="tight", dpi=300)
        if return_fig: return fig
        else: plt.close(fig); return None
    except Exception as e: print(f"❌ Error in comparative map: {e}"); return None
...
def analyze_tiff_statistics(tiff_path: str, return_fig=False):
    try:
        ...
        # Adjust bins to avoid a distorted histogram due to massive outliers
        ...
        plt.hist(clipped_values, bins=50, density=True, alpha=0.7, color='skyblue', label='Data Distribution (P1 to P99)')
        x_norm = np.linspace(clipped_values.min(), clipped_values.max(), 100)
        pdf_fitted = norm.pdf(x_norm, mean_val, std_val)
        plt.plot(x_norm, pdf_fitted, 'r-', linewidth=2, label=f'Normal Distribution (μ={mean_val:.2e}, σ={std_val:.2e})')
        plt.title(f"Value Distribution for {Path(tiff_path).stem}")
        plt.xlabel("Pixel Value"); plt.ylabel("Density"); plt.legend(); plt.grid(True, linestyle='--', alpha=0.7)
...
def run_processing(params, cancel_event):
    ...
    print(f"\n--- 🗓️  PROCESSING {gas_name} in {region_nombre.replace('_', ' ').title()} for {time_start} to {time_end} ---")
    ...
    if ruta_crudo_gas and ruta_crudo_gas.exists():
        ...
        if not params.get('do_comparative_map', False):
            ...
            if params['generar_mapas'] and (fig := generar_mapa_con_leyenda(ruta_gas_para_procesar, aoi_path, params['title_date'], params['year'], cmap=params['palette_gas'], producto=f"{gas_name} (Column)", unidad="(mol/m²)", return_fig=params['show_plots'])): params['fig_queue'].put(fig)
    ...
    if params.get('transform_method') == 'petetin':
        print(f"\n🌍 Downloading BLH (H) data for: {params['title_date']} {params['year']}")
        ...
        if bounding_box:
            ...
            if exito_descarga:
                ...
                if ruta_blh_para_analisis and not params.get('do_comparative_map', False):
                    ...
                    if params['generar_mapas'] and (fig := generar_mapa_con_leyenda(ruta_blh_para_analisis, aoi_path, params['title_date'], params['year'], cmap=params['palette_blh'], producto="Boundary Layer (BLH)", unidad="(m)", return_fig=params['show_plots'])): params['fig_queue'].put(fig)
    ...
    if can_calculate:
        folder_leaf = "Savenets Equation" if params['transform_method'] == 'savanets' else (f"{params.get('transform_value', 0)} transformed" if params['transform_method'] == 'custom' else params.get('output_name_blh', params['output_name']).replace('BLH', 'Concentration').replace(f'Datos_{gas_name}', 'Concentration'))
        calc_output_dir = BASE_OUTPUT_PATH / f"Calculations_{gas_name}/{params['year']}/{region_nombre}/{folder_leaf}"
        ...
        # --- EXTRACT PPB TIME SERIES IF REQUESTED ---
        if params.get('generar_serie') and daily_tiffs:
            csv_ppb_name = calc_output_dir / f"Time_Series_Range_PPB_{gas_name}_{params['title_date']}.csv"
        ...
        if ruta_concentracion_final and not params.get('do_comparative_map', False):
            ...
            if params['generar_mapas'] and (fig := generar_mapa_con_leyenda(ruta_concentracion_final, aoi_path, params['title_date'], params['year'], cmap=params['palette_transform'], producto=f"{gas_name} Concentration", unidad="(ppb)", return_fig=params['show_plots'])): params['fig_queue'].put(fig)

    # Always clean the temp directory after finished process
    if temp_dir and temp_dir.exists():
        try: shutil.rmtree(temp_dir, ignore_errors=True)
        except Exception: pass

    if params['do_zip'] and ruta_crudo_gas: comprimir_directorio(ruta_crudo_gas.parent, ruta_crudo_gas.parent.parent / f"{ruta_crudo_gas.parent.name}_{gas_name}-raw.zip")
...
def run_processing_ratio(params, cancel_event):
    """Executes the specialized pipeline to calculate the spatial ratio between two gases."""
    matplotlib.use('Agg')
    gas_num_info, gas_den_info = params['gas_num_info'], params['gas_den_info']
    aoi_path, region_nombre = params['aoi_path'], Path(params['aoi_path']).stem
    
    print(f"\n==========================================================")
    print(f"🔬 MULTI-GAS ANALYSIS: RATIO {gas_num_info['nombre_corto']} / {gas_den_info['nombre_corto']}")
    print(f"   Analyzing local photochemical sensitivity (VOC vs NOx)...")
    print(f"==========================================================")
    
    # 1. Download Numerator Gas (E.g. HCHO)
    ...
    # 2. Download Denominator Gas (E.g. NO2)
    ...
    # 4. Generate Statistics, Correlation Matrix and Maps
    if ruta_ratio and not params.get('do_comparative_map', False):
        ...
        if params['generar_mapas']:
            # We suggest a diverging scale if default (Spectral helps a lot with HCHO/NO2 transition)
            cmap = "Spectral_r" if params['palette_gas'] in ['viridis', 'plasma', 'inferno'] else params['palette_gas']
            fig = generar_mapa_con_leyenda(ruta_ratio, aoi_path, params['title_date'], params['year'], cmap=cmap, producto=f"Ratio {gas_num_info['nombre_corto']}/{gas_den_info['nombre_corto']}", unidad="(dimensionless)", return_fig=params['show_plots'])
...
# ==============================================================================
# SECTION 2: TKINTER APPLICATION CLASS
# ==============================================================================
class GeoApp:
    def __init__(self, root, available_regions):
        self.root = root
        self.root.title("Multi-Gas Atmospheric Data Processor")
    ...
    def create_gas_widgets(self, parent):
        frame = ttk.LabelFrame(parent, text="0. Gas Selection / Ratio Maps", padding="10")
        ...
        self.gas_notebook.add(self.tab_sub_blh, text="Sub-BLH Gases (Transformable)")
        self.gas_notebook.add(self.tab_post_blh, text="Post-BLH Gases (Column Only)")
        self.gas_notebook.add(self.tab_ratio, text="Ratio Maps (e.g., HCHO/NO2)")
        
        ttk.Label(self.tab_sub_blh, text="Gas (Confined in PBL):").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(self.tab_post_blh, text="Gas (Extended Troposphere):").pack(side=tk.LEFT, padx=5)
        ...
        # --- Ratio Maps UI ---
        ttk.Label(self.tab_ratio, text="Numerator:").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(self.tab_ratio, text=" / Denominator:").pack(side=tk.LEFT, padx=5)
    ...
    def on_gas_tab_changed(self, event):
        ...
        if current_tab == 1 or current_tab == 2: # Post BLH or Ratio
    ...
    def create_date_widgets(self, parent):
        frame = ttk.LabelFrame(parent, text="1. Date Selection", padding="10")
        ...
        ttk.Radiobutton(radio_frame, text="Specific Month", variable=self.date_choice, value="mes", command=self.toggle_date_widgets).pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(radio_frame, text="Full Year", variable=self.date_choice, value="anio", command=self.toggle_date_widgets).pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(radio_frame, text="Specific Day", variable=self.date_choice, value="dia", command=self.toggle_date_widgets).pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(radio_frame, text="Day Range", variable=self.date_choice, value="rango", command=self.toggle_date_widgets).pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(radio_frame, text="Month Range", variable=self.date_choice, value="rango_meses", command=self.toggle_date_widgets).pack(side=tk.LEFT, padx=5)
        
        self.date_widgets_frame = ttk.Frame(frame); self.date_widgets_frame.grid(row=1, column=0, columnspan=4, sticky="w", pady=5)
        
        self.mes_label, self.mes_combo = ttk.Label(self.date_widgets_frame, text="Month:"), ttk.Combobox(self.date_widgets_frame, values=list(meses_dict.values()), state="readonly", width=self.ancho_combobox_mes); self.mes_combo.set("January")
        self.ano_label, self.ano_spin = ttk.Label(self.date_widgets_frame, text="Year:"), tk.Spinbox(self.date_widgets_frame, from_=2018, to=2030, width=self.ancho_spinbox_ano)
        self.ano_label_completo, self.ano_spin_completo = ttk.Label(self.date_widgets_frame, text="Year:"), tk.Spinbox(self.date_widgets_frame, from_=2018, to=2030, width=self.ancho_spinbox_ano)
        self.dia_label, self.dia_cal = ttk.Label(self.date_widgets_frame, text="Date:"), DateEntry(self.date_widgets_frame, date_pattern='yyyy-mm-dd', width=12)
        self.rango_label1, self.rango_cal1 = ttk.Label(self.date_widgets_frame, text="From:"), DateEntry(self.date_widgets_frame, date_pattern='yyyy-mm-dd', width=12)
        self.rango_label2, self.rango_cal2 = ttk.Label(self.date_widgets_frame, text="To:"), DateEntry(self.date_widgets_frame, date_pattern='yyyy-mm-dd', width=12)
        self.rango_mes_label_ini, self.rango_mes_combo_ini, self.rango_ano_spin_ini = ttk.Label(self.date_widgets_frame, text="From:"), ttk.Combobox(self.date_widgets_frame, values=list(meses_dict.values()), state="readonly", width=self.ancho_combobox_mes), tk.Spinbox(self.date_widgets_frame, from_=2018, to=2030, width=self.ancho_spinbox_ano); self.rango_mes_combo_ini.set("January")
        self.rango_mes_label_fin, self.rango_mes_combo_fin, self.rango_ano_spin_fin = ttk.Label(self.date_widgets_frame, text="To:"), ttk.Combobox(self.date_widgets_frame, values=list(meses_dict.values()), state="readonly", width=self.ancho_combobox_mes), tk.Spinbox(self.date_widgets_frame, from_=2018, to=2030, width=self.ancho_spinbox_ano); self.rango_mes_combo_fin.set("March")
    ...
    def create_region_widgets(self, parent):
        frame = ttk.LabelFrame(parent, text="2. Region Selection", padding="10")
        ...
        ttk.Radiobutton(rb_frame, text="Select from Preloaded List", variable=self.region_mode, value="list", command=self.toggle_region_mode).pack(side=tk.LEFT, padx=10)
        ttk.Radiobutton(rb_frame, text="Manual Coordinates (BBox)", variable=self.region_mode, value="manual", command=self.toggle_region_mode).pack(side=tk.LEFT, padx=10)
        ...
        ttk.Label(self.frame_list, text="Region:").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(nm_frame, text="Zone Name (no spaces):").pack(side=tk.LEFT)
        ...
        ttk.Label(coord_frame, text="Min Longitude (W):").grid(row=0, column=0, padx=5, pady=2, sticky="e"); ttk.Entry(coord_frame, textvariable=self.min_lon, width=10).grid(row=0, column=1, padx=5, pady=2)
        ttk.Label(coord_frame, text="Max Longitude (E):").grid(row=0, column=2, padx=5, pady=2, sticky="e"); ttk.Entry(coord_frame, textvariable=self.max_lon, width=10).grid(row=0, column=3, padx=5, pady=2)
        ttk.Label(coord_frame, text="Min Latitude (S):").grid(row=1, column=0, padx=5, pady=2, sticky="e"); ttk.Entry(coord_frame, textvariable=self.min_lat, width=10).grid(row=1, column=1, padx=5, pady=2)
        ttk.Label(coord_frame, text="Max Latitude (N):").grid(row=1, column=2, padx=5, pady=2, sticky="e"); ttk.Entry(coord_frame, textvariable=self.max_lat, width=10).grid(row=1, column=3, padx=5, pady=2)
    ...
    def create_options_widgets(self, parent):
        proc_frame = ttk.LabelFrame(parent, text="3. Processing Options", padding="10")
        ...
        self.transform_checkbutton = ttk.Checkbutton(proc_frame, text="Transform to surface", variable=self.do_transform, command=self.toggle_transform_options)
        ...
        self.transform_method_var = tk.StringVar(value="Download BLH and Calculate Concentration (H. Petetin Mode)")
        self.transform_method_combo = ttk.Combobox(self.transform_options_frame, textvariable=self.transform_method_var, values=["Download BLH and Calculate Concentration (H. Petetin Mode)", "Savanets Equation (10km altitude)", "Value manually entered by user"], state="readonly", width=50)
        self.transform_method_combo.pack(anchor="w")
        
        ttk.Checkbutton(proc_frame, text="Rescale original data (Kriging)", variable=self.do_regrid).pack(anchor="w")
        ttk.Checkbutton(proc_frame, text="Analyze Statistics (Distribution & Correlation)", variable=self.do_stats).pack(anchor="w")
        ttk.Checkbutton(proc_frame, text="Generate Individual Maps", variable=self.do_maps).pack(anchor="w")
        self.comp_map_checkbutton = ttk.Checkbutton(proc_frame, text="Generate Comparative Map (Year/Month Range only)", variable=self.do_comparative_map)
        self.comp_map_checkbutton.pack(anchor="w")
        
        self.timeseries_checkbutton = ttk.Checkbutton(proc_frame, text="Plot Time Series (Median & P5-P95 Range)", variable=self.do_timeseries)
        self.timeseries_checkbutton.pack(anchor="w")
        
        ttk.Checkbutton(proc_frame, text="Compress original results when finished", variable=self.do_zip).pack(anchor="w")
        
        ttk.Separator(proc_frame, orient='horizontal').pack(fill='x', pady=10)
        ttk.Label(proc_frame, text="Output Format (Raster):").pack(anchor='w', pady=(5,0))
        ...
        vis_frame = ttk.LabelFrame(parent, text="4. Visualization", padding="10")
        vis_frame.grid(row=3, column=1, sticky="nsew", padx=5, pady=5)
        self.show_plots = tk.BooleanVar(value=True)
        ttk.Checkbutton(vis_frame, text="Show generated plots/maps", variable=self.show_plots).pack(anchor="w")
        ttk.Separator(vis_frame, orient='horizontal').pack(fill='x', pady=10)
        
        paleta_opciones = list(paletas_colores.keys())
        ttk.Label(vis_frame, text="Column/Ratio Gas Color:").pack(anchor='w')
        ...
        ttk.Label(vis_frame, text="BLH Map Color:").pack(anchor='w', pady=(5,0))
        ...
        ttk.Label(vis_frame, text="Transformed Gas Color:").pack(anchor='w', pady=(5,0))
        ...
        ttk.Separator(vis_frame, orient='horizontal').pack(fill='x', pady=10)
        ttk.Label(vis_frame, text="Quick Cloud Cover Check:", font=("Default", 9, "bold")).pack(anchor='w', pady=(0, 5))
        cloud_frame = ttk.Frame(vis_frame); cloud_frame.pack(fill=tk.X)
        self.btn_cloud = ttk.Button(cloud_frame, text="☁️ Calculate Cloud %", command=self.on_cloud_click); self.btn_cloud.pack(side=tk.LEFT, padx=(0, 10))
    ...
    def run_cloud_check(self, params):
        try:
            ...
            if not t_start or not t_end or not route: self.update_cloud_ui("Insuf. Data", "red"); return
    ...
    def create_action_buttons(self, parent):
        ...
        self.run_button = ttk.Button(frame, text="Start Processing", command=self.start_processing_thread); self.run_button.pack(side=tk.RIGHT, padx=5)
        self.btn_download_all = ttk.Button(frame, text="Download all regions", command=self.start_batch_processing_thread); self.btn_download_all.pack(side=tk.RIGHT, padx=5)
        self.cancel_button = ttk.Button(frame, text="Cancel", command=self.request_cancellation, state="disabled"); self.cancel_button.pack(side=tk.RIGHT, padx=5)
        ttk.Button(frame, text="Exit", command=self.on_closing).pack(side=tk.RIGHT)
    ...
    def start_processing_thread(self):
        ...
        self.clear_console()
        print("--- STARTING PROCESS (Single Region) ---")
        try:
    ...
    def start_batch_processing_thread(self):
        self.cancel_event.clear(); self.run_button.config(state="disabled"); self.btn_download_all.config(state="disabled"); self.cancel_button.config(state="normal")
        self.clear_console(); print("--- STARTING BATCH PROCESS ---")
        try:
    ...
    def start_batch_processing(self, base_params, cancel_event):
        try:
            if not self.available_regions: print("❌ No regions available."); return
            print(f"📋 {len(self.available_regions)} regions to process.")
            for i, reg in enumerate(self.available_regions):
                if cancel_event.is_set(): break
                print(f"\n\n>>> 🌍 PROCESSING REGION {i+1}/{len(self.available_regions)}: {reg} <<<")
    ...
    def request_cancellation(self):
        print("\n--- 🛑 CANCELLATION REQUESTED ---"); self.cancel_event.set(); self.cancel_button.config(state="disabled")

    def _get_month_list(self, y1, m1, y2, m2):
        months = []; f_ini, f_fin = datetime(y1, m1, 1), datetime(y2, m2, 1)
        if f_ini > f_fin: raise ValueError("Start date is after end date.")
        c_y, c_m = y1, m1
        while True:
    ...
    def get_params(self, ignore_region=False):
        ...
        # Identify Selected Gas or Ratio Mode
        current_tab = self.gas_notebook.index("current")
        if current_tab == 0:
        ...
        if choice == "mes":
            ...
            params['title_date'] = meses_en_lower[mes_num]
            params['output_name'] = f"{mes_num:02d}_Data_{gas_name}_{params['title_date']}_{ano}"
            params['output_name_blh'] = f"{mes_num:02d}_Data_BLH_{params['title_date']}_{ano}"
            ...
            params['output_name'], params['title_suffix'] = f"Data_{gas_name}_Day_{fecha}", f"Day {fecha}"
            params['folder_period'] = fecha

        elif choice == "rango":
            start_date_obj, end_date_obj = self.rango_cal1.get_date(), self.rango_cal2.get_date()
            if start_date_obj > end_date_obj: raise ValueError("Start cannot be after end date.")
            ...
            params['title_date'], params['title_suffix'] = f"Range {start_date} to {end_date}", f"Range {start_date} to {end_date}"
            params['output_name'], params['output_name_blh'] = f"Data_{gas_name}_Range_{start_date}_to_{end_date}", f"Data_BLH_Range_{start_date}_to_{end_date}"
            params['folder_period'] = f"{start_date}_to_{end_date}"

        elif choice == "anio":
            ano = int(self.ano_spin_completo.get())
            params['year'], params['month_list'], params['title_suffix'] = ano, [(ano, m) for m in range(1, 13)], f"Year {ano}"
            params['folder_period'] = "Full_Year"
            ...
            params['month_list'], params['year'], params['title_suffix'] = self._get_month_list(y1, m1, y2, m2), y1, f"Range {self.rango_mes_combo_ini.get()} {y1} - {self.rango_mes_combo_fin.get()} {y2}"
        ...
        if not ignore_region:
            if self.region_mode.get() == "list":
                if not self.region_combo.get(): raise ValueError("Please select a region.")
                params['aoi_path'] = BASE_GEOJSON_PATH / f"{self.region_combo.get()}.geojson"
                if not params['aoi_path'].exists(): raise FileNotFoundError("GeoJSON not found.")
            else:
                nm = "".join([c for c in self.manual_name_var.get().strip() if c.isalnum() or c in (' ', '_', '-')]).replace(" ", "_")
                if not nm: raise ValueError("Please enter a zone name.")
                min_x, min_y, max_x, max_y = self.min_lon.get(), self.min_lat.get(), self.max_lon.get(), self.max_lat.get()
                if min_x >= max_x or min_y >= max_y: raise ValueError("Minimums must be smaller than maximums.")
        ...
        if self.do_transform.get() and not params['is_ratio']:
            ...
            elif "User-defined" in selection:
                val = simpledialog.askfloat("BLH Height", "Enter height in meters:\n(E.g., 1000)", parent=self.root, minvalue=1.0, maxvalue=50000.0)
            ...
            if params.get('is_ratio'):
                if params.get('choice_mode') in ['anio', 'rango_meses']:
                    ...
                    for year, month in params['month_list']:
                        if cancel_event.is_set(): break 
                        p_m = params.copy()
                        mes_nombre_en = meses_en_lower[month]
                        _, last_day = calendar.monthrange(year, month)
                        p_m.update({'start_date': f"{year}-{month:02d}-01", 'end_date': f"{year}-{month:02d}-{last_day}", 'year': year, 'month': month, 'title_date': mes_nombre_en, 'output_name': f"{month:02d}_Data_{params['gas_num_info']['nombre_corto']}_{mes_nombre_en}_{year}"})
                ...
                if params.get('choice_mode') in ['anio', 'rango_meses']:
                    gas_name = params['gas_info']['nombre_corto']
                    print(f"\n==========================================================")
                    print(f"🚀 MULTI-MONTH PROCESS: {params['title_suffix']} | GAS: {gas_name}")
                    print(f"==========================================================")
                    ...
                    for year, month in b_params['month_list']:
                        ...
                        mes_nombre_en = meses_en_lower[month]
                        _, last_day = calendar.monthrange(year, month)
                        p_m.update({'start_date': f"{year}-{month:02d}-01", 'end_date': f"{year}-{month:02d}-{last_day}", 'year': year, 'month': month, 'title_date': mes_nombre_en, 'output_name': f"{month:02d}_Data_{gas_name}_{mes_nombre_en}_{year}"})
                        p_m['output_name_blh'] = f"{month:02d}_Data_BLH_{mes_nombre_en}_{year}" if p_m['descargar_blh'] else f"Transformed_Data_{p_m.get('transform_method')}"
                    ...
                    if params['do_comparative_map'] and not cancel_event.is_set():
                        ...
                        out_dir_col = BASE_OUTPUT_PATH / f"Model_{gas_name}/{params['year']}/{region_nombre}/{params['folder_period']}"
                        out_dir_col.mkdir(parents=True, exist_ok=True)
                        if no2_paths and (fig := generar_mapa_comparativo(no2_paths, params['aoi_path'], f"{gas_name} (Column)", "(mol/m²)", params['palette_gas'], params['title_suffix'], output_dir=out_dir_col, return_fig=params['show_plots'])):
                            if fq: fq.put(fig)
                        ...
                        if blh_paths and params['descargar_blh'] and (fig := generar_mapa_comparativo(blh_paths, params['aoi_path'], "Boundary Layer (BLH)", "(m)", params['palette_blh'], params['title_suffix'], output_dir=out_dir_blh, return_fig=params['show_plots'])):
                            if fq: fq.put(fig)
                            
                        out_dir_ppb = BASE_OUTPUT_PATH / f"Calculations_{gas_name}/{params['year']}/{region_nombre}/{params['folder_period']}"
                        out_dir_ppb.mkdir(parents=True, exist_ok=True)
                        if conc_paths and params.get('transform_method') and (fig := generar_mapa_comparativo(conc_paths, params['aoi_path'], f"{gas_name} Concentration", "(ppb)", params['palette_transform'], params['title_suffix'], output_dir=out_dir_ppb, return_fig=params['show_plots'])):
                            if fq: fq.put(fig)
                    ...
                    if params['generar_serie'] and not cancel_event.is_set():
                        folder_period = params.get('folder_period', 'Period')
                        if csv_paths:
                            out_dir_col = BASE_OUTPUT_PATH / f"Model_{gas_name}/{params['year']}/{region_nombre}/{folder_period}"
                            out_dir_col.mkdir(parents=True, exist_ok=True)
                        ...
                        if csv_paths_ppb:
                            out_dir_ppb = BASE_OUTPUT_PATH / f"Calculations_{gas_name}/{params['year']}/{region_nombre}/{folder_period}"
                            out_dir_ppb.mkdir(parents=True, exist_ok=True)
    ...
    def processing_finished(self):
        print("\n\n🛑 STOPPED!" if self.cancel_event.is_set() else "\n\n✅ FINISHED!")
...
if __name__ == "__main__":
    if not BASE_GEOJSON_PATH.is_dir():
        messagebox.showerror("Configuration Error", f"The 'Regiones' folder was not found.\nPlease make sure it exists at:\n{SCRIPT_DIR}")
        sys.exit(1)

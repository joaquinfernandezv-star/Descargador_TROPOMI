...
import json
import zipfile
from datetime import datetime, timedelta # Necesario para el rango de meses, iteración y parseo
...
try:
    ...
    import cdsapi
    import rioxarray
    import xarray as xr # <--- Importación de xarray para el promedio local
...
try:
    config = SHConfig()
    config.download_timeout_seconds = 120 # Se reduce porque ahora las peticiones son diarias y rápidas
...
# --- Evalscript SIMPLE (Único a usar ahora) ---
evalscript_raw = """
//VERSION=3
...
def descargar_y_promediar_no2(time_start, time_end, route, output_name, cancel_event):
    """
    Descarga datos de NO2 día por día y luego calcula el promedio usando xarray.
    """
    ...
    final_folder = BASE_OUTPUT_PATH / f"Modelo/{year_folder}/{area}/{output_name}"
    ...
    # Si es solo un día, lo descargamos directo a la carpeta final
    if delta.days == 0:
        request = SentinelHubRequest(
            ...
            config=config,
            data_folder=str(final_folder)
        )
        request.get_data(save_data=True)
        filenames = request.get_filename_list()
        if filenames:
            return (final_folder / filenames[0]).resolve()
        return None

    # Si es un rango de días, iteramos día por día
    temp_dir = BASE_OUTPUT_PATH / "Temp_Daily" / output_name
    temp_dir.mkdir(parents=True, exist_ok=True)
    
    daily_tiffs = []
    print(f"🔄 Iniciando descarga diaria progresiva ({delta.days + 1} días)...")
    ...
    for i in range(delta.days + 1):
        ...
        # Mensaje de progreso
        sys.stdout.write(f"\r      Descargando y procesando día: {current_date}... ")
        sys.stdout.flush()
        ...
        request = SentinelHubRequest(
            ...
            input_data=[SentinelHubRequest.input_data(
                data_collection=data_5p,
                time_interval=(current_date, current_date),
                other_args={'processing': {'upsampling': 'NEAREST', 'minQa': '75', 'timeliness': 'OFFL'}}
            )],
            responses=[SentinelHubRequest.output_response("default", MimeType.TIFF)],
            bbox=BBox(bbox=aoi_coords, crs=CRS.WGS84).transform(CRS(3857)),
        ...
        try:
            ...
            # Días nublados o sin datos fallarán en silencio (evitamos saturar la consola)
            pass

    print("\n✅ Descargas diarias finalizadas. Promediando datos localmente con xarray...")

    if not daily_tiffs:
        print("❌ No se encontraron datos válidos en ningún día del período.")
        shutil.rmtree(temp_dir, ignore_errors=True)
        return None
    ...
    try:
        ...
        for tiff in daily_tiffs:
            try:
                ...
                # Reemplazamos nodata por np.nan para que el promedio se calcule correctamente ignorando huecos
        ...
        # Concatenar todos los días en la dimensión 'time'
        combined = xr.concat(datasets, dim='time')
        # Calcular el promedio a lo largo de la dimensión 'time' ignorando NaNs
        mean_ds = combined.mean(dim='time', skipna=True)
        
        # Restaurar sistema de coordenadas y nodata
        ref_ds = datasets[0]
        ...
        print(f"✅ Promedio local guardado con éxito en la carpeta final.")
        
    except Exception as e:
        print(f"❌ Error al procesar el promedio con xarray: {e}")
        shutil.rmtree(temp_dir, ignore_errors=True)
        return None
    ...
    try:
        ...
        print(f"🧹 Archivos diarios temporales eliminados correctamente.")
    except Exception:
        pass
        
    return final_tiff_path.resolve()
...
def generar_mapa_con_leyenda(ruta_tiff, ruta_geojson, title_date, year, cmap='viridis', alpha=0.75, producto="NO₂", unidad="(mol/m²)", return_fig=False):
    try:
        ...
        if 'regrid' in Path(ruta_tiff).stem.lower(): procesamiento = "Re-escalado (Kriging)"
        elif 'concentracion' in Path(ruta_tiff).stem.lower(): procesamiento = "Calculado"
        else: procesamiento = "Crudo"
...
def generar_mapa_comparativo(file_paths, aoi_path, producto, unidad, cmap, title_suffix, return_fig=False):
    ...
    try:
        ...
        if target_crs and gdf.crs != target_crs: gdf_plot = gdf.to_crs(target_crs)
        else: gdf_plot = gdf
        ...
        if img: 
            ...
            fig.supxlabel('Longitud', fontsize=16, y=0.02) 
            fig.supylabel('Latitud', fontsize=16, x=0.01)

        else: fig.tight_layout(rect=[0.04, 0.05, 1.0, 0.95])
...
def analyze_tiff_statistics(tiff_path: str, return_fig=False):
    try:
        ...
        stats_df = pd.DataFrame({
            ...
            'Value': [
                np.nanmin(pixel_values), np.nanmax(pixel_values), 
                f"{median_val:.3e}", f"{mean_val:.3e}", f"{std_val:.3e}", pixel_values.size
            ]
        })
...
def run_processing(params, cancel_event):
    ...
    # Delegamos la descarga diaria y el promedio con xarray a la nueva función
    ruta_crudo_no2 = descargar_y_promediar_no2(time_start, time_end, aoi_path, params['output_name'], cancel_event)

    if ruta_crudo_no2 and ruta_crudo_no2.exists():
        print(f"🗂️  Archivo base NO₂ (crudo/promediado): {ruta_crudo_no2}")
        ...
        if params['do_regrid']:
            ...
            if ruta_regrid_no2_str:
                ...
                print(f"⚠️  Advertencia: El re-escalado falló. Se continuará con el archivo original.")
        ...
        if not bounding_box:
            ...
            if params.get('choice_mode') == 'dia':
                 ...
                 if 'month' in params:
                     ...
                     print("⚠️ Modo no compatible con descarga BLH automática.")
    ...
    if can_calculate:
        ...
        file_suffix = ""
        if params['do_regrid'] and ruta_no2_para_procesar.name != "response.tiff":
            # Just add regrid suffix if it was actually regridded
            file_suffix = "_regrid"
    ...
    if params['do_zip'] and ruta_crudo_no2:
        source_folder_to_zip = ruta_crudo_no2.parent
